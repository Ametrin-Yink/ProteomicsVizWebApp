...
class TestDuckDBStreaming:
    ...
    def test_output_matches_pandas(self, tmp_path):
        ...
        processor = DataProcessor(ProcessingConfig(remove_razor=False, strict_filtering=False))
        ...
        assert not missing_group, f"DuckDB output missing group columns: {missing_group}"
        ...
        abund_diff = (merged["Abundance_dd"].astype(float) - merged["Abundance_pd"].astype(float)).abs()
        assert abund_diff.max() < 0.01, (
    ...
    def test_contaminant_filter(self, tmp_path):
        ...
        assert len(result) == 5, f"Expected 5 rows after contam filter, got {len(result)}"
    ...
    def test_low_abundance_filter(self, tmp_path):
        ...
        result = pd.read_parquet(output_path)
        assert len(result) == 7, f"Expected 7 rows after abundance filter, got {len(result)}"
