...
test.describe('Data Input', () => {
  ...
  test('uploads compound file', async ({ page }) => {
    // Upload proteomics files first (required before compound)
    await uploadFiles(page, [
      '../../SampleData/PSM_SampleData_DMSO_1.csv',
    ]);
    ...
    // Verify compound upload success (use specific locator within compound-info)
    await expect(page.locator('[data-testid="compound-info"] [data-testid="compound-upload-success"]').first()).toBeVisible({ timeout: 15000 });
  ...
  test('compound file upload succeeds', async ({ page }) => {
    ...
    // Verify compound upload success message appears (use specific locator within compound-info)
    await expect(page.locator('[data-testid="compound-info"] [data-testid="compound-upload-success"]').first()).toBeVisible({ timeout: 15000 });
