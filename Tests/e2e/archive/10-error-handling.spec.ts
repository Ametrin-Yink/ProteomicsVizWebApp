/**
 * E2E Test Suite 10: Error Handling
 *
 * Tests all error scenarios with specific error messages.
 * CRITICAL: Errors must be clear and actionable.
 */
...
test.beforeAll(() => {
  purgeLegacyScreenshots('10-error-handling');
});
...
test.describe('Error Handling', () => {
  ...
  test('invalid filename format shows clear error', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'invalid-filename-error', 'error-shown');
  });
  ...
  test('missing required columns shows specific error', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'missing-columns-error', 'error-shown');
  });
  ...
  test('insufficient replicates prevents starting', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'insufficient-replicates', 'validation-shown');
  });
  ...
  test('missing config prevents starting', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'missing-config', 'error-shown');
  });
  ...
  test('404 page for deleted session', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'deleted-session-404', 'error-shown');
  });
  ...
  test('network error handled gracefully', async ({ page }) => {
    ...
    await takeScreenshot(page, '10-error-handling', 'network-error-handled', 'graceful');
  ...
  test('file too large shows error', async ({ page }) => {
    ...
    // Create a large file (this is a mock - actual test would need real large file)
    // For now, test the error display mechanism
    await page.evaluate(() => {
    ...
    // If error display works, great
    if (errorVisible) {
      const errorText = await page.locator('[data-testid="toast-error"]').textContent();
      expect(errorText).toMatch(/size|large|limit|500MB/i);
    }

    await takeScreenshot(page, '10-error-handling', 'file-size-error', 'error-shown');
  });
});
