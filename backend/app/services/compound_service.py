...
class CompoundService:
    ...
    def parse_compound_csv(self, file_path: Path) -> dict[str, CompoundData]:
        ...
        if df.empty:
            raise ValidationError(
                message="Compound CSV file is empty", details={"file": str(file_path)}
            )
        ...
        smiles_col = self._find_column(
            df.columns, ["smiles", "structure", "mol", "molecule"]
        )
        if not smiles_col:
            raise ValidationError(
        ...
        condition_col = self._find_column(
            df.columns, ["condition", "treatment", "sample", "group"]
        )
        ...
        for _, row in df.iterrows():
            ...
            if mol is None:
                logger.warning(
                    f"Invalid SMILES for Corp ID {corp_id}: {smiles[:50]}..."
                )
                continue
    ...
    def _find_column(
        ...
        """
        columns_lower = {
            col.lower().replace("_", " ").replace("-", " "): col for col in columns
        }
    ...
    def get_compound_with_structure(
        ...
        structure_image = self.generate_structure_image(compound.smiles, format=format)
        ...
        if format.lower() == "svg":
            ...
            if structure_image:
                result["structure_png_base64"] = base64.b64encode(
                    structure_image
                ).decode("utf-8")
            else:
    ...
    def get_all_compounds(
        self, compound_file: Path, include_structures: bool = False, format: str = "svg"
    ) -> list[dict[str, Any]]:
        """
        ...
        for corp_id, compound in compounds.items():
            ...
            if include_structures:
                ...
                if format.lower() == "svg":
                    ...
                    if structure_image:
                        entry["structure_png_base64"] = base64.b64encode(
                            structure_image
                        ).decode("utf-8")
                    else:
