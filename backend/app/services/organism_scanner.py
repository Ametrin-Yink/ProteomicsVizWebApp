...
class OrganismScanner:
    ...
    def scan(self) -> list[dict[str, str]]:
        ...
        for fasta_file in fasta_files:
            ...
            gene_mapping_file_v1 = (
                self.protein_database_dir / f"{organism_id}_uniprot_gene.tsv"
            )
            gene_mapping_file_v2 = (
                self.protein_database_dir / f"{organism_name}_GeneName.tsv"
            )

            # Check if gene mapping file exists (either format)
            if gene_mapping_file_v1.exists() or gene_mapping_file_v2.exists():
                organisms.append({"id": organism_id, "name": organism_name})
    ...
    def get_organism_path(self, organism_id: str) -> dict[str, Path]:
        ...
        if fasta_v2.exists() and gene_v2.exists():
            return {"fasta": fasta_v2, "gene_mapping": gene_v2}

        # Fall back to original naming convention
        return {
            "fasta": self.protein_database_dir / f"{organism_id}.fasta",
            "gene_mapping": self.protein_database_dir
            / f"{organism_id}_uniprot_gene.tsv",
        }
