...
class PlotGenerator:
    ...
    async def load_data(
        ...
        """
        self.diff_data = pd.read_csv(diff_expression_path, sep='\t')
        
        if protein_abundances_path and protein_abundances_path.exists():
            self.protein_data = pd.read_csv(protein_abundances_path, sep='\t')
    
    # =========================================================================
    # Volcano Plot Functions
    # =========================================================================
    
    def prepare_volcano_data(self, diff_expression_df: pd.DataFrame) -> dict[str, Any]:
        """
        ...
        protein_col = 'Protein' if 'Protein' in diff_expression_df.columns else 'master_protein_accessions'
        ...
        df['significant'] = (df[pval_col] < pval_threshold) & (abs(df[logfc_col]) > logfc_threshold)
        ...
        return {
            ...
            'protein': df[protein_col].tolist() if protein_col in df.columns else [''] * len(df),
    ...
    def generate_pca_plot(
        ...
        for i, sample in enumerate(pca_data.samples):
            ...
            ax.scatter(pca_data.pc1[i], pca_data.pc2[i],
                      c=[color_map[condition]], s=100, alpha=0.7,
                      edgecolors='black', linewidth=1)
            
            # Add sample labels
            ax.annotate(sample, (pca_data.pc1[i], pca_data.pc2[i]),
        ...
        # Legend
        legend_elements = [Patch(facecolor=color_map[c], edgecolor='black',
                                label=c) for c in conditions]
    ...
    def generate_pvalue_distribution_plot(
        ...
        if counts:
            ...
            ax.axhline(y=expected, color='red', linestyle='--',
                      alpha=0.5, label='Uniform distribution')
            ax.legend()
    ...
    def generate_cv_plot(
        ...
        ax.set_title('PSM Coefficient of Variation by Condition', 
    ...
    def generate_data_completeness_plot(
        ...
        if completeness_data:
            ...
            ax.bar(x, missing, width_bar, bottom=present, 
                  label='Missing', color='#E74C3C')
            
            ax.set_xlabel('Sample', fontsize=12)
            ax.set_ylabel('Number of Proteins', fontsize=12)
            ax.set_title('Data Completeness by Sample', 
                        fontsize=14, fontweight='bold')
            ax.set_xticks(x)
    ...
    def generate_gsea_bar_plot(
        ...
        if not significant:
            ax.text(0.5, 0.5, 'No significant pathways found',
            ...
            # Prepare data
            names = [r.name[:50] + '...' if len(r.name) > 50 else r.name 
                    for r in top]
            nes_values = [r.nes for r in top]
            colors = [self.colors['upregulated'] if n > 0 
                     else self.colors['downregulated'] for n in nes_values]
            ...
            ax.set_title(f'GSEA: {gsea_results.database}\nTop {len(top)} Significant Pathways',
                        fontsize=14, fontweight='bold')
    ...
    def generate_qc_report(
        ...
        if qc_data.pvalue_distribution:
            ...
            self.generate_pvalue_distribution_plot(
                qc_data.pvalue_distribution.model_dump(),
                pval_path, dpi=dpi
            )
        ...
        if qc_data.intensity_distributions:
            ...
            self.generate_intensity_distribution_plot(
                qc_data.intensity_distributions.model_dump(),
                intensity_path, dpi=dpi
            )
    ...
    async def generate_heatmap_data(
        ...
        abundance_cols = [
            col for col in self.protein_data.columns
            if col not in id_cols and self.protein_data[col].dtype in ['float64', 'float32']
        ]
