...
class DataProcessor:
    ...
    def _select_best_protein(
        ...
        candidate_counts = {
            p: peptide_counts.get(p, 0) for p in proteins
        }
    ...
    def step4_remove_low_quality(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Step 4 complete: Removed {removed} low quality PSMs, "
            f"{len(df)} remaining"
        )
    ...
    def step5_filter_by_criteria(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"  Using {'strict' if self.config.strict_filtering else 'lenient'} filtering (threshold={threshold})")
