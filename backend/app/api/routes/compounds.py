...
async def get_compound_structure(
    ...
    try:
        ...
        if not session.files or not session.files.compound:
            raise HTTPException(
                status_code=404,
                detail="No compound file uploaded for this session"
            )
        ...
        if not compound_file.exists():
            raise HTTPException(
                status_code=404,
                detail="Compound file not found"
            )
        ...
        if compound_data is None:
            raise HTTPException(
                status_code=404,
                detail=f"No compound found for condition: {condition}"
            )
...
async def get_compound_image(
    ...
    try:
        ...
        if structure_image is None:
            raise HTTPException(
                status_code=500,
                detail="Failed to generate structure image"
            )
...
async def get_compound_properties(
    ...
    try:
        ...
        if properties is None:
            raise HTTPException(
                status_code=500,
                detail="Failed to calculate molecular properties"
            )
