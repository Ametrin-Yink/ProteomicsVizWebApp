...
def _build_sample_filter(session, comparison: str) -> list[str] | None:
    ...
    for comp in comparisons:
        g1 = comp.get("group1", {})
        g2 = comp.get("group2", {})
        g1_str = "+".join(str(v) for v in g1.values())
        g2_str = "+".join(str(v) for v in g2.values())
        if f"{g1_str}_vs_{g2_str}" == comparison:
            prefixes = [str(v) for v in list(g1.values()) + list(g2.values())]
            return prefixes
    return None
...
async def load_diff_expression_results(
    ...
    try:
        ...
        if results and all(r["psm_count"] == 0 for r in results):
            protein_abundance_file = results_dir / "Protein_Abundances.tsv"
            if protein_abundance_file.exists():
                try:
                    pa_df = await asyncio.to_thread(
                        pd.read_csv, protein_abundance_file, sep="\t"
                    )
                    if "PSM_Count" in pa_df.columns and "Master_Protein_Accessions" in pa_df.columns:
                        psm_lookup = dict(
                            zip(
...
async def get_gsea_heatmap_data(
    ...
    if sample_filter and not protein_df.empty:
        metadata_cols = [
            ...
            if not any(c.startswith(prefix) for prefix in sample_filter)
            and c
            in {
        ...
        sample_cols = [
            ...
            if any(c.startswith(prefix) for prefix in sample_filter)
        ]
        protein_df = protein_df[metadata_cols + sample_cols]
