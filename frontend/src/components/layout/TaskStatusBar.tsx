"use client";

import { useState, useEffect, useRef } from "react";
import { visualizationApi } from "@/lib/api-client";

interface TaskInfo {
  kind: string;
  label: string;
  status: "queued" | "running" | "completed" | "error" | "cancelled";
  started_at: string | null;
  completed_at: string | null;
  error: string | null;
  progress: { completed: number; total: number } | null;
  queue_position: number | null;
}

interface TaskStatus {
  tasks: TaskInfo[];
}

function getCurrentSessionId(): string | null {
  if (typeof window === "undefined") return null;
  // Check URL path first (e.g., /sessions/<id>/...)
  const pathMatch = window.location.pathname.match(/\/sessions\/([^/]+)/);
  if (pathMatch) return pathMatch[1];
  // Check query params (e.g., ?session_id=<id>)
  const params = new URLSearchParams(window.location.search);
  return params.get('session_id') || params.get('session');
}

const STATUS_STYLE: Record<string, string> = {
  running: "bg-info/10 text-info",
  queued: "bg-warning/10 text-warning",
};

function statusLabel(task: TaskInfo): string {
  if (task.status === "running") return "Running";
  if (task.status === "queued") return `Queued #${task.queue_position || "?"}`;
  return task.status;
}

function progressPct(task: TaskInfo): number {
  if (!task.progress || task.progress.total <= 0) return 0;
  return (task.progress.completed / task.progress.total) * 100;
}

export function TaskStatusBar() {
  const [status, setStatus] = useState<TaskStatus>({ tasks: [] });
  const [expanded, setExpanded] = useState(false);
  const lastPayload = useRef("");

  useEffect(() => {
    let active = true;
    const timeoutRef = { current: null as ReturnType<typeof setTimeout> | null };

    const poll = async () => {
      if (!active) return;
      try {
        const sessionId = getCurrentSessionId();
        if (sessionId) {
          const data = await visualizationApi.getTaskStatus(sessionId);
          const payload = JSON.stringify(data);
          if (payload !== lastPayload.current) {
            lastPayload.current = payload;
            setStatus(data);
          }
        }
      } catch {
        // silently ignore fetch errors
      }
      if (active) {
        timeoutRef.current = setTimeout(poll, 2000);
      }
    };

    poll();

    return () => {
      active = false;
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
    };
  }, []);

  const handleCancel = async () => {
    try {
      const sessionId = getCurrentSessionId();
      if (!sessionId) return;
      await visualizationApi.cancelTasks(sessionId);
    } catch {
      // silently ignore
    }
  };

  const activeTasks = status.tasks.filter(
    (t) => t.status === "running" || t.status === "queued"
  );

  const hasActive = activeTasks.length > 0;

  return (
    <div className="fixed bottom-4 right-4 z-50">
      {expanded ? (
        <div className="bg-surface border border-border rounded-lg shadow-lg p-4 min-w-[320px] max-w-[420px]">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-text-primary">
              {hasActive ? `Tasks (${activeTasks.length})` : "Tasks"}
            </h3>
            <button
              onClick={() => setExpanded(false)}
              className="text-text-muted hover:text-text-primary text-sm"
            >
              ✕
            </button>
          </div>
          {hasActive ? (
            <>
              {activeTasks.map((task, i) => (
                <div
                  key={`${task.kind}-${task.started_at}-${i}`}
                  className="mb-2 last:mb-0 p-2 bg-background rounded"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-text-primary truncate max-w-[200px]">
                      {task.label || task.kind}
                    </span>
                    <span className={`text-xs px-2 py-0.5 rounded ${STATUS_STYLE[task.status] || "bg-error/10 text-error"}`}>
                      {statusLabel(task)}
                    </span>
                  </div>
                  {task.progress && task.status === "running" && (
                    <div className="mt-1 w-full bg-border rounded-full h-1.5">
                      <div
                        className="bg-info h-1.5 rounded-full transition-all"
                        style={{ width: `${progressPct(task)}%` }}
                      />
                    </div>
                  )}
                  {task.error && (
                    <p className="text-xs text-error mt-1 truncate">{task.error}</p>
                  )}
                </div>
              ))}
              {activeTasks.some((t) => t.status === "running") && (
                <button
                  onClick={handleCancel}
                  className="mt-3 w-full text-xs text-error border border-error/30 hover:border-error rounded px-3 py-1"
                >
                  Cancel All
                </button>
              )}
            </>
          ) : (
            <p className="text-xs text-text-muted">
              No active tasks. Trigger GSEA or compare analysis to see status here.
            </p>
          )}
        </div>
      ) : (
        <button
          onClick={() => setExpanded(true)}
          className="bg-surface border border-border rounded-full shadow-lg p-3 relative hover:bg-background"
        >
          <svg className="w-5 h-5 text-text-secondary" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zm0-2a8 8 0 110-16 8 8 0 010 16zm-1-5h2v2h-2v-2zm0-8h2v6h-2V7z" />
          </svg>
          <span className="absolute -top-1 -right-1 bg-info text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
            {activeTasks.length}
          </span>
        </button>
      )}
    </div>
  );
}
